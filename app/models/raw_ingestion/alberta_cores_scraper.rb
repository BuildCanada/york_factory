class RawIngestion::AlbertaCoresScraper < ActiveRecord::AssociatedObject
  include CorporateNormalization
  include ProvincialScraping

  performs :scrape

  SEARCH_URL = "https://cores.reg.gov.ab.ca/cores/cr/cr_search.html"

  def scrape(search_names: nil)
    rows_processed = 0

    names = search_names || seed_names_for_province("AB")
    ...
    names.each_with_index do |name, idx|
      ...
      begin
        ...
        Rails.logger.error "[AlbertaCoresScraper] Error searching '#{name}': #{e.message}"
        save_scraping_progress("last_error", { name: name, error: e.message, at: Time.current.iso8601 })
      end
    end

    raw_ingestion.update!(status: :complete)
    Rails.logger.info "[AlbertaCoresScraper] Processed #{rows_processed} AB corps for ingestion #{raw_ingestion.id}"
  rescue => e
  ...
  def search_registry(name)
    ...
    search_field = form.fields.find { |f| f.name =~ /name|search|query/i }
    ...
    Rails.logger.warn "[AlbertaCoresScraper] Search failed for '#{name}': #{e.message}"
    []
  end
  ...
  def map_result(result)
...

    {
      ...
      registered_office_province: "AB",
      source_system: "ab_cores",
