class RawIngestion::OntarioObrScraper < ActiveRecord::AssociatedObject
  include CorporateNormalization
  include ProvincialScraping

  performs :scrape

  SEARCH_URL = "https://www.ontario.ca/page/ontario-business-registry"

  def scrape(search_names: nil)
    rows_processed = 0

    names = search_names || seed_names_for_province("ON")
    ...
    names.each_with_index do |name, idx|
      ...
      begin
        ...
        Rails.logger.error "[OntarioObrScraper] Error searching '#{name}': #{e.message}"
        save_scraping_progress("last_error", { name: name, error: e.message, at: Time.current.iso8601 })
      end
    end

    raw_ingestion.update!(status: :complete)
    Rails.logger.info "[OntarioObrScraper] Processed #{rows_processed} ON corps for ingestion #{raw_ingestion.id}"
  rescue => e
  ...
  def search_registry(name)
    ...
    search_field = form.fields.find { |f| f.name =~ /search|query|name/i }
    ...
    Rails.logger.warn "[OntarioObrScraper] Search failed for '#{name}': #{e.message}"
    []
  end
  ...
  def parse_results(page)
    ...
    page.search("table tr, .search-result, .result-item").each do |row|
      cells = row.search("td, .field-value, span")
  ...
  def map_result(result)
...

    {
      ...
      registered_office_province: "ON",
      source_system: "on_obr",
