class Warehouse::MediaArticle < Warehouse::Record
  include Searchable
  ...
  def self.import!(feed:, feed_entry:, extraction:, normalizer: Search::Media::ArticleNormalizer.new)
    attempts = 0
    begin
      attributes = normalizer.call(feed:, feed_entry:, extraction:)
      article = find_or_initialize_by(
        canonical_url_digest: Digest::SHA256.hexdigest(attributes.fetch(:canonical_url))
      )
      attributes = attributes.except(:media_feed_id) if article.persisted?
