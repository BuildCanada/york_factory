class Warehouse::RawIngestion::RelationshipLoader < ActiveRecord::AssociatedObject
  ...
  def load(csv_content:)
    ...
    da_index = Warehouse::GeoBoundary.by_type(:da).pluck(:geo_uid, :id).to_h
    parent_indices = RELATIONSHIP_COLUMNS.each_with_object({}) do |(col, type), hash|
      parent_type = type.sub("da_", "")
      hash[col] = Warehouse::GeoBoundary.by_type(parent_type).pluck(:geo_uid, :id).to_h
    end
    ...
    Warehouse::GeoRelationship.upsert_all(
      records,
