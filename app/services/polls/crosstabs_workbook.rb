    ...
    def render
      ...
      report["tables"].each_with_index do |table, index|
        book.add_worksheet(name: "Q#{index + 1}") do |sheet|
          ...
          sheet.add_row [ "Question #{index + 1}\n#{english(table["question"])}" ], style: styles[:title], height: text_height(english(table["question"]), 35 + columns.size * 14) + 32
          ...
          variants.each do |arm, variant|
            wording = if variant.is_a?(Hash) && variant.key?("prompt")
              ([ variant["prompt"], variant["description"], *Array(variant["options"]&.values), *variant.values_at("minLabel", "maxLabel", "trueLabel", "falseLabel") ].compact.map { |value| english(value) }).reject(&:blank?).join("\n")
            else
              english(variant)
            end
          ...
          Array(table["arms"]).each_with_index do |arm, arm_index|
            sheet.add_row []
            heading = "Version #{arm_index + 1}: #{english(arm["question"])}"
            sheet.add_row [ heading ], style: styles[:heading], height: text_height(heading, 35 + columns.size * 14)
